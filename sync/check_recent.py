...
Local:    fill in sync/.env, then run `python sync/check_recent.py` from repo root
...
# Make sure `import leetcode_client` works regardless of where this is run from.
sys.path.insert(0, str(Path(__file__).parent))
 
from dotenv import load_dotenv
load_dotenv(Path(__file__).parent / ".env")  # no-op if the file doesn't exist (e.g. in Actions)
